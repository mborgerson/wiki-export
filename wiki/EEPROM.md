...
bytes). Checksum2 was correct since the extra 4 bytes not used in the
CRC computation were all 0's which would not change the computed CRC
value. However, Simliar problem with computation of Checksum3, the extra
4 bytes used are all 0's on all but v1.6 Xbox's. The CRC computed for
v1.6 Xbox's is incorrect as the 4 extra bytes are not 0's as on earlier
versions.
